...
- **诊断表自动刷新间隔 20 秒 → 6 小时**（`DIAG_REFRESH_MS = 6 * 60 * 60 * 1000`）：
  渠道状态变化很慢，秒级轮询既无信息量又白刷接口/日志；打开诊断页仍**立即刷新一次**，
  「刷新自检」按钮随时可手动刷新，6 小时只是兜底。界面文案同步改为
  「每 6 小时自动刷新（打开诊断页会立即刷新一次）（最近 …）」，单测锁住间隔值禁止回退。
...
- **实测证据（2026-09-19，dsh 0.1.6-alpha.1 / 面板 3080 + 真实 Chromium 点击）**：
  ...
  - 单测：`node --test scripts/unit.test.mjs scripts/client.test.mjs` = 68 项全绿
    （新增 `scripts/client.test.mjs` 10 项：修复结果文案的成功/部分/跳过/无返回四态 + 一键汇总 + 6 小时刷新间隔）。
