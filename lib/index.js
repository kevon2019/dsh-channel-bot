...
const chatServices = (platform, chatId, chatType) => ({
  ...
  const startWechatPolling = (cfg) => {
    ...
    console.log("[channel-bot] wechat long-poll started");
    const loop = async () => {
      ...
      try {
        ...
        if (!cur.enabled || !cur.wechat?.enabled || !tok) {
          ...
          if (wechatPolling) wechatTimer = setTimeout(loop, WECHAT_IDLE_MS);
          return;
        }
        wechatGapLogged = false;
      ...
      },
      {
        ...
        handler: (_request, response) => {
          ...
          sendJson(response, 200, {
            ...
            health: {
              telegram: healthyView("telegram", cfg, { enabled: !!(cfg.enabled && cfg.telegram?.enabled), tokenSet: !!cfg.telegram?.botToken, pollerRunning: pollTimer !== null }),
              wecom: healthyView("wecom", cfg, { enabled: !!(cfg.enabled && cfg.wecom?.enabled), tokenSet: !!(cfg.wecom?.botId && cfg.wecom?.secret), pollerRunning: wecomWs !== null }),
              qq: healthyView("qq", cfg, { enabled: !!(cfg.enabled && cfg.qq?.enabled), tokenSet: !!(cfg.qq?.appId && cfg.qq?.appSecret), pollerRunning: true }),
              wechat: healthyView("wechat", cfg, { enabled: !!(cfg.enabled && cfg.wechat?.enabled), tokenSet: !!cfg.wechat?.botToken, pollerRunning: wechatPolling }),
            },
            state: {
