...
async function sendWecom(cfg, text) {
  ...
  const wx = cfg.wecom || {};
  if (wx.corpid && wx.corpsecret && wx.agentid && wx.touser) {
...
async function sendQq(cfg, target, text) {
  const base = cfg.qq?.onebotUrl;
  if (!base) return;
  const headers = { "content-type": "application/json" };
  if (cfg.qq?.accessToken) headers.authorization = `Bearer ${cfg.qq.accessToken}`;
...
async function testChannel(cfg, channel) {
  ...
  switch (channel) {
    ...
    case "wecom": {
      ...
      if (!appMsg && !wx.outWebhook) throw new Error("未配置企业微信（需填 4 项应用消息参数 或 Webhook 地址）");
  ...
  const sendToChat = async (binding, text) => {
    ...
    for (const chunk of chunks) {
      try {
        ...
        else if (binding.platform === "wechat") await sendWechat(cfg, { channel: "wechat", chatId: binding.chatId }, chunk);
        ...
        else if (binding.platform === "wecom") await sendWecom(cfg, chunk);
      } catch (error) {
...
const chatServices = (platform, chatId, chatType) => ({
  ...
  const applyConfig = (cfg) => { startPolling(cfg); startWechatPolling(cfg); };
