...
async function testChannel(cfg, channel, scheme) {
  ...
  switch (channel) {
    ...
    case "wecom": {
      ...
      if (scheme === "2") {
        ...
        if (!target) throw new Error("企业微信长连接已认证，但尚无目标会话；请先在企微向该机器人发一条消息以建立会话");
      ...
      /* 未指定：优先方案一，否则方案二 */
      if (appMsg || wx.outWebhook) { await sendWecom(cfg, text); break; }
      if (wx.botId && wx.secret) {
        if (!wecomWs) throw new Error("企业微信长连接客户端未连接，请稍后重试");
        const target = wx.touser || wecomChatId;
        if (!target) throw new Error("企业微信长连接已认证，但尚无目标会话；请先在企微向机器人发一条消息");
        await wecomWs.sendMessage(String(target), { msgtype: "markdown", markdown: { content: text } });
        break;
      }
    ...
    case "qq": {
      ...
      if (scheme === "2" || (!scheme && q.appId && q.appSecret)) {
        /* 方案二：QQ 开放平台 v2（appId+appSecret） */
      ...
      if (scheme === "1") {
        ...
        if (!q.groupId) throw new Error("未配置方案一（QQ 群号，测试发到群）");
      ...
      },
      {
        ...
        handler: async (request, response) => {
          try {
            ...
            await testChannel(current(), channel, scheme);
