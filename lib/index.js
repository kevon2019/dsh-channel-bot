...
async function sendWecom(cfg, text) {
  ...
  if (wx.botId && wx.secret && wecomWs) {
    const chatId = wx.touser || wecomChatId;
  ...
  const cb = cfg.wecom || {};
  if (cb.corpid && cb.corpsecret && cb.agentid && cb.touser) {
    ...
    const body = { touser: wx.touser, agentid: Number(wx.agentid), msgtype: "text", text: { content: text }, safe: 0 };
...
async function testChannel(cfg, channel) {
  ...
  switch (channel) {
    ...
    case "wecom": {
      ...
      if (!appMsg && !wx.outWebhook) {
        /* 方案二：企微智能机器人长连接（botId+secret）。若客服已连上 Bot 客户端，
         * 直接向最近会话推一条测试消息；无目标会话则提示先给机器人发一条消息。 */
        if (wx.botId && wx.secret) {
          if (!wecomWs) throw new Error("企业微信长连接客户端未连接（正在初始化/重连，请稍后重试）");
          const target = wx.touser || wecomChatId;
          if (target) {
            await wecomWs.sendMessage(String(target), { msgtype: "markdown", markdown: { content: text } });
            console.log("[channel-bot] wecom 长连接测试已发 target=", target);
            break;
          }
          throw new Error("企业微信长连接已认证，但尚无目标会话；请先在企微向该机器人发一条消息以建立会话");
        }
        throw new Error("未配置企业微信（需填 4 项应用消息参数、Webhook 地址、或方案二 Bot ID+Secret）");
      }
    ...
    case "qq": {
      ...
      if (q.appId && q.appSecret) {
        /* v2 开放平台：先验证凭据可换 access_token，再向最近收到过消息的会话做真实被动回复
         * （带其 openid + msg_id）。未收到过消息则给出清晰指引。 */
        ...
        const firstId = qqMsgIdMap.keys().next().value;
        if (!firstId) throw new Error("QQ 开放平台 v2 凭据验证通过，但尚无已收到消息的会话；请先在 QQ 给机器人发一条消息");
        await sendQq(cfg, { type: "private", id: String(firstId) }, text);
        break;
      }
    ...
    case "wechat": {
      const w = cfg?.wechat || {};
      const uid = w.notifyUserId || w.allowedUserIds?.[0];
      if (!uid) throw new Error("未配置微信接收用户 ID（通知用户 ID 或允许的用户 ID）");
      ...
      },
      {
        ...
        handler: async (request, response) => {
          try {
            ...
            await testChannel(current(), channel);
