...
async function testChannel(cfg, channel, scheme, target) {
  ...
  switch (channel) {
    ...
    case "qq": {
      ...
      if (scheme === "2" || (!scheme && q.appId && q.appSecret)) {
        /* 方案二：QQ 开放平台 v2（appId+appSecret），目标 = 指定 target(openid) || 最近收到消息的会话 */
        ...
        if (!id) throw new Error("QQ 开放平台 v2 凭据验证通过，但尚无目标会话；请填一个「目标ID」(对方openid) 或先在 QQ 给机器人发一条消息");
        await sendQq(cfg, { type: "private", id: String(id) }, text);
        break;
      }
      if (scheme === "1") {
        /* 方案一：OneBot HTTP */
        ...
        await sendQq(cfg, { type: "group", id: String(id) }, text);
        break;
      }
      ...
      },
      {
        ...
        handler: async (request, response) => {
          try {
            ...
            await testChannel(current(), channel, scheme, target);
