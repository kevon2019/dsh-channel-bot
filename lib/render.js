...
const REDACT_PATTERN = /(Bearer\s+[A-Za-z0-9._-]+|sk-[A-Za-z0-9_-]{8,}|AKIA[0-9A-Z]{16}|gh[pousr]_[A-Za-z0-9_-]{12,}|eyJ[A-Za-z0-9._-]{20,})/g;
/* [2026-09-18 v2.0.0] .test() 必须用**不带 /g** 的等价正则：带 /g 的正则对象在 .test()
 * 之后会保留 lastIndex，下一次 .test() 从上次位置继续 → 同一个字符串会时而命中、时而漏判，
 * 漏判就是「凭证原样进了远程审批卡片」。replace 用 /g 版本，判定用无状态版本。 */
const REDACT_TEST = new RegExp(REDACT_PATTERN.source);
  ...
  const walk = (value, depth = 0) => {
    ...
    if (typeof value === "object") {
      ...
      const shown = entries.slice(0, 8).map(([k, v]) => {
        ...
        if (typeof v === "string" && REDACT_TEST.test(v)) return `${k}=***`;
