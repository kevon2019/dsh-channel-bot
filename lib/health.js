...
export const STATE_FILE = join(homedir(), ".dsh", "channel-bot-state.json");
...
export function savePersistedState() {
  ...
  saveTimer = setTimeout(() => {
    saveTimer = null;
    try { writeFileSync(STATE_FILE, JSON.stringify(persistedState, null, 2)); } catch { /* 落盘失败不影响收发 */ }
  }, 800);
}
...
export function healthyView(ch, cfg, info) {
  ...
  if (info.enabled && r.poller.fails > 0 && r.poller.lastError) problems.push("最近轮询报错：" + r.poller.lastError);
  ...
  if (info.enabled && info.tokenSet && info.pollerRunning && r.inbound.count === 0) {
    notes.push("本次运行尚未收到过入站消息，可用「测试验证」发一条或从手机发一条来确认接收链路");
  }
  return {
