  ...
  function ChannelBotSection() {
    ...
    return React.createElement(FoldContext.Provider, { value: foldValue },
      ...
      React.createElement(SettingsGroup, { title: "QQ (OneBot)", hint: "OneBot HTTP 上报 + 出站。", group: "cb-qq" },
        ...
        React.createElement(SectionField, { label: "方案二 · OpenClaw 机器人模式（可选）", hint: "QQ 在 OpenClaw 生态（如 OpenClaw 的 QQ 连接）中填入 Bot ID + Secret。此方案仅作配置记录/展示，实际 QQ 对话由 OpenClaw 生态承接，本插件不直接收发 QQ 智能机器人消息（上方方案一 OneBot 不受影响）。" },
      ...
      React.createElement(SettingsGroup, { title: "微信（个人号 · 官方 iLink）", hint: "腾讯官方 iLink Bot API，扫码登录，长轮询收消息，无封号风险。", group: "cb-wc" },
        ...
        React.createElement(SectionField, { label: "方案二 · OpenClaw 机器人模式（可选）", hint: "微信在 OpenClaw 生态（如 OpenClaw 的官方微信/iLink/ClawBot）中填入 Bot ID + Secret。此方案仅作配置记录/展示，实际微信对话由 OpenClaw 生态承接，本插件不直接收发微信智能机器人消息（上方方案一 iLink 不受影响）。" },
