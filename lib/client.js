...
window.__ModuleLoader__.load({ id: "dsh-channel-bot", factory: (require) => {
  ...
  function ChannelBotSection() {
    ...
    return React.createElement(FoldContext.Provider, { value: foldValue },
      ...
      React.createElement(SettingsGroup, { title: "诊断" },
        React.createElement(Row, { label: "全渠道双向自检 + 分渠道修复", hint: "入站 = 收消息，出站 = 回复/推送；六个渠道各自的接收通道状态、最近入站与出站结果，可单独自检（发一条测试消息）。每条渠道还能「修复」：自动重启它的接收通道（轮询/长连接）、清除该渠道的访问令牌缓存与历史错误计数，并真打一次平台接口校验凭据；顶部「一键修复全部」按顺序修复所有已启用渠道。下表每 20 秒自动刷新。" }),
  ...
  function ChannelHealthBlock(props) {
    ...
    React.useEffect(function () { load(); var iv = setInterval(load, 20000); return function () { clearInterval(iv); }; }, []);
