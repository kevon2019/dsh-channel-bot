...
window.__ModuleLoader__.load({ id: "dsh-channel-bot", factory: (require) => {
  ...
  /* ---------- 设置不可用自诊断（v2.0.0-rc.6，v2.0.0 复核 0.1.6） ----------
   * [2026-09-10 实测 / 2026-09-18 复核] dsh >= 0.1.5 在「非本机托管」（域名 / 反向代理访问）时，
   * 核心客户端把 settings 降级为 unavailable：ctx.settingsScope.bind() 永远拿不到
   * status==="ready" 的快照，凡是只依赖 settings.section 的插件就会无限显示
   * 「加载中…」。根因在核心（@deepseek-ai/dsh-client-ui-settings-models 会直接抛
   * "settings are unavailable in this browser"），插件无法自救，但可以「超时后
   * 给出可操作的诊断」，而不是让用户面对一个永远转圈的页面。
   * 0.1.6-alpha.1 复核：本机（loopback）托管时设置作用域正常 ready；该降级行为仍在。
   */
  var SETTINGS_TIMEOUT_MS = 8000;
  var rebindHook = null;                            // apply() 注册，供「重新绑定」按钮
  var hostDiag = { isLoopback: null, origin: "" };  // apply() 采集
   ...
   *
   * [2026-09-10] 合并必须基于「写时刻的实时快照」，不能用渲染期 props：同一块里连续
   * 提交两个字段（如方案二 Bot ID + Secret）时，第二次提交携带的还是旧 value，
   * 会把第一次刚写的字段覆盖回去（实测：secret 落盘成功、botId 被清空）。 */
  function liveChannel(bound, value, ch) {
    try {
      var snap = bound !== null && bound.scope !== null && typeof bound.scope.getSnapshot === "function"
        ? bound.scope.getSnapshot() : null;
      if (snap !== null && snap.status === "ready" && snap.value !== null && typeof snap.value === "object"
          && snap.value[ch] !== null && typeof snap.value[ch] === "object") return snap.value[ch];
    } catch { /* fall through to render-time props */ }
    return (value && value[ch]) || {};
  }
  function writeChannel(bound, value, ch, patch) {
    write(bound, ch, Object.assign({}, liveChannel(bound, value, ch), patch));
  }
  ...
  function ChannelActions({ group, channel, top, bound, value, testable, hint, scheme, targetable }) {
    var [saveText, setSaveText] = React.useState("");
    var [testText, setTestText] = React.useState("");
    var [target, setTarget] = React.useState((value && value[channel] && value[channel].testTargetId) || "");
    var [ttype, setTtype] = React.useState((value && value.qq && value.qq.testTargetType) || "private");
    var persistTarget = (v) => { if (channel) writeChannel(bound, value, channel, { testTargetId: v.trim() }); };
    var persistTtype = (v) => { if (channel) writeChannel(bound, value, channel, { testTargetType: v }); };
    var doSave = () => {
    ...
    var doTest = () => {
      ...
      if (targetable) {
        if (target) payload.target = target.trim();
        if (channel === "qq") payload.targetType = ttype;
      }
      fetch("/api/channel-bot/test", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload), cache: "no-store" })
        .then((r) => r.json())
        .then((b) => setTestText(b && b.ok ? (b.message ? "✅ " + b.message : "✅ 已发出，请到「" + channel + "」确认收到") : "❌ " + (b && b.error || "发送失败")))
    ...
    return React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", margin: "8px 0 4px" } },
      ...
      testable && targetable ? React.createElement("input", { type: "text", value: target, onChange: (e) => setTarget(e.target.value), onBlur: () => persistTarget(target), "aria-label": "测试目标 ID", placeholder: "目标ID (存设置自动带出/填对方ID)", style: { width: 190, padding: "5px 8px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l2,#333)", background: "transparent", color: "inherit", fontSize: 12 } }) : null,
      testable && channel === "qq" ? React.createElement("select", { value: ttype, onChange: (e) => { setTtype(e.target.value); persistTtype(e.target.value); }, "aria-label": "测试目标类型", style: { padding: "5px 8px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l2,#333)", background: "transparent", color: "inherit", fontSize: 12 } },
        React.createElement("option", { value: "private" }, "私聊"),
        React.createElement("option", { value: "group" }, "群聊")) : null,
  ...
  /* ---------- 获取凭据指引 ---------- */
  function CredHelp({ label, steps, href, hrefLabel }) {
    var [open, setOpen] = React.useState(false);
    var link = href ? React.createElement("a", { href: href, target: "_blank", rel: "noopener noreferrer", style: { color: "var(--dsw-alias-brand-primary, #315efb)", fontSize: 12, marginLeft: 8, textDecoration: "none" } }, hrefLabel || "打开后台") : null;
    var body = open ? React.createElement("div", { style: { margin: "6px 0 2px", padding: "8px 10px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l2, #333)", background: "rgba(127,127,127,0.08)", fontSize: 12, lineHeight: 1.7, color: "var(--dsw-alias-text-secondary, #7f8a99)" } },
      React.createElement("div", null, "从哪里获取配置："),
      steps.map(function (s, i) { return React.createElement("div", { key: i }, "  " + (i + 1) + ". " + s); })) : null;
    return React.createElement("div", { style: { display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6, margin: "6px 0 0" } },
      React.createElement("button", { type: "button", "aria-label": "获取凭据指引", onClick: function () { setOpen(!open); }, style: { padding: "4px 8px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l2, #333)", background: "transparent", color: "inherit", fontSize: 12, cursor: "pointer" } }, (open ? "收起指引" : "🙋 获取凭据指引")),
      link, body);
  }
  ...
  /* ---------- 设置不可用：自诊断面板（替代无限「加载中…」） ---------- */
  function SettingsUnavailablePanel(props) {
    var retryCount = props.retryCount || 0;
    var [copied, setCopied] = React.useState(false);
    var hit = hostDiag.isLoopback === true ? "loopback（本机托管）"
      : hostDiag.isLoopback === false ? "remote（非本机托管：域名 / 反向代理）" : "未知";
    var btn = { padding: "6px 12px", borderRadius: 8, border: "1px solid var(--dsw-alias-border-l2, #333)",
      background: "transparent", color: "inherit", cursor: "pointer", fontSize: 12 };
    var code = { fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 11,
      padding: "1px 5px", borderRadius: 4, background: "var(--dsw-alias-fill-l2, rgba(127,127,127,.16))" };
    var hint = "当前地址 " + (hostDiag.origin || "?") + " 被核心判定为 " + hit + "。"
      + "dsh 把插件设置的读写托管在核心 settings 服务上，非本机托管时核心会静默降级为 unavailable，"
      + "于是所有基于 settings.section 的插件都会卡在这里。这是核心行为，与插件版本无关。";
    return React.createElement("div", { style: { padding: 12, fontSize: 13, lineHeight: 1.7,
        border: "1px dashed var(--dsw-alias-border-l2, #333)", borderRadius: 10 } },
      React.createElement("div", { style: { fontWeight: 600, fontSize: 14, marginBottom: 6 } },
        "设置不可用：等待 " + Math.round(SETTINGS_TIMEOUT_MS / 1000) + " 秒仍未取得设置作用域"
        + (props.retrying ? "（已重新绑定，等待中…）" : "")),
      React.createElement("div", null, hint),
      React.createElement("div", { style: { marginTop: 10, fontWeight: 600 } }, "两条修复路径（任选其一）"),
      React.createElement("div", null, "1）本机访问：用 ", React.createElement("span", { style: code }, "http://127.0.0.1:3080/?token=…"), " 打开面板（设置只在本机托管下可写）。"),
      React.createElement("div", null, "2）域名部署：给核心客户端打放行补丁，让部署域名被视同本机托管（", React.createElement("span", { style: code }, "isLoopback"), " 一处判断），插件设置即可正常读写。"),
      React.createElement("div", { style: { marginTop: 10, fontSize: 12, opacity: 0.7 } },
        "机器人本身仍在后台运行（渠道连接不受影响），此处仅影响面板里的设置读写。"),
      React.createElement("div", { style: { marginTop: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" } },
        React.createElement("button", { type: "button", style: btn, onClick: props.onRetry },
          retryCount > 0 ? "重新绑定（已试 " + retryCount + " 次）" : "重新绑定"),
        React.createElement("button", { type: "button", style: btn, onClick: function () {
          var text = "dsh-channel-bot 设置不可用诊断\n地址: " + (hostDiag.origin || "?")
            + "\nisLoopback: " + String(hostDiag.isLoopback)
            + "\n等待: " + SETTINGS_TIMEOUT_MS + "ms 未取到 settings 作用域"
            + "\ndsh-core-isLoopback-判定在非本机托管时降级 settings（0.1.5 / 0.1.6 实测）"
            + "\n提示: 点「测试状态」可看到核心版本与本插件版本（status.compat）";
          try { void navigator.clipboard.writeText(text); setCopied(true); } catch { /* clipboard 不可用时忽略 */ }
        } }, copied ? "已复制 ✓" : "复制诊断信息")));
  }
  ...
  function ChannelBotSection() {
    ...
    /* [2026-09-10] 超时自诊断：作用域迟迟不就绪时不再无限「加载中…」（见 SettingsUnavailablePanel） */
    var [waited, setWaited] = React.useState(0);
    var [retry, setRetry] = React.useState(0);
    React.useEffect(function () {
      if (value !== null) { setWaited(0); return void 0; }
      var t0 = Date.now();
      var iv = setInterval(function () {
        var ms = Date.now() - t0;
        var v = ms > SETTINGS_TIMEOUT_MS ? SETTINGS_TIMEOUT_MS : ms;
        setWaited(function (w) { return v > w ? v : w; });   /* 单调递增：重试时不会退回 spinner */
        if (ms > SETTINGS_TIMEOUT_MS) clearInterval(iv);
      }, 400);
      return function () { clearInterval(iv); };
    }, [value === null, retry]);
    if (value === null) {
      if (waited < SETTINGS_TIMEOUT_MS) {
        return React.createElement("div", { style: { color: "var(--dsw-alias-text-tertiary, #888)", fontSize: 13, padding: 12 } }, "多渠道机器人设置加载中…");
      }
      return React.createElement(SettingsUnavailablePanel, {
        retrying: retry > 0,
        retryCount: retry,
        onRetry: function () {
          setRetry(function (r) { return r + 1; });
          if (rebindHook !== null) { try { rebindHook(); } catch { /* ignore */ } }
        }
      });
    }
    var testStatus = () => {
    ...
    var CA = (group, channel, top, hint, scheme, targetable) => React.createElement(ChannelActions, { group, channel, top, testable: !!channel, bound, value, hint, scheme, targetable });
    ...
    return React.createElement(FoldContext.Provider, { value: foldValue },
      ...
      CA("cb-fs", "feishu"),
      React.createElement(SettingsGroup, { title: "企业微信", hint: "群机器人 Webhook；填 4 项应用消息参数则发给指定成员个人。方案二为智能机器人长连接（直接收发）。", group: "cb-wx" },
        React.createElement(Row, { label: "启用" },
        ...
        React.createElement("div", { "data-save-group": "cb-wx-s1" },
          React.createElement(SectionField, { label: "方案一 · 机器人 Webhook 地址", hint: "企业微信群机器人 Webhook，填入即可向该群推送。" },
            React.createElement(Field, { value: (value.wecom && value.wecom.outWebhook) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { outWebhook: v }), ariaLabel: "企业微信 Webhook", field: "outWebhook", fieldType: "text", placeholder: "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=..." })),
          React.createElement(SectionField, { label: "方案一 · 企业微信应用消息（个人，填了优先于上面 Webhook）", hint: "发给指定成员个人：企业 ID / 应用 Secret / AgentId / 接收成员 ID。" },
            React.createElement(Field, { value: (value.wecom && value.wecom.corpid) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { corpid: v }), ariaLabel: "企业 ID (corpid)", field: "corpid", fieldType: "text", placeholder: "企业微信 corpid（管理后台「我的企业」）" }),
            React.createElement(Field, { value: (value.wecom && value.wecom.corpsecret) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { corpsecret: v }), ariaLabel: "应用 Secret (corpsecret)", field: "corpsecret", fieldType: "text", placeholder: "应用的 Secret（管理后台应用详情）" }),
            React.createElement(Field, { value: (value.wecom && value.wecom.agentid) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { agentid: v }), ariaLabel: "应用 AgentId", field: "agentid", fieldType: "text", placeholder: "应用 AgentId（管理后台应用详情）" }),
            React.createElement(Field, { value: (value.wecom && value.wecom.touser) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { touser: v }), ariaLabel: "接收成员ID (touser)", field: "touser", fieldType: "text", placeholder: "接收成员 UserID（多个用 | 分隔）" })),
          CA("cb-wx-s1", "wecom", null, "保存 / 测试方案一", "1", true)),
        React.createElement("div", { "data-save-group": "cb-wx-s2" },
          React.createElement(SectionField, { label: "方案二 · 企业微信智能机器人 · 长连接", hint: "在企微管理后台「安全与合规 → 管理工具 → 智能机器人」创建 API 模式机器人并选「长连接」，生成 Bot ID + Secret 填入。本插件以此直接长连接收发企微机器人消息（@wecom/aibot-node-sdk），无需公网回调。填了后优先于上方方案一。" },
            React.createElement(Field, { value: (value.wecom && value.wecom.botId) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { botId: v }), ariaLabel: "企业微信智能机器人 Bot ID", field: "botId", fieldType: "text", placeholder: "智能机器人 Bot ID（创建后自动生成，形如 ww...）" }),
            React.createElement(Field, { value: (value.wecom && value.wecom.secret) || "", onCommit: (v) => writeChannel(bound, value, "wecom", { secret: v }), ariaLabel: "企业微信智能机器人 Secret", field: "secret", fieldType: "text", placeholder: "智能机器人 Secret（管理后台复制）" })),
          React.createElement(CredHelp, { label: "企微智能机器人", href: "https://work.weixin.qq.com", hrefLabel: "打开企微管理后台", steps: ["登录企业微信管理后台 WorkWeixin（work.weixin.qq.com）：管理工具 → 智能机器人", "点击「创建机器人」，接入方式选「API 模式 · 长连接」", "创建后复制生成的 Bot ID 与 Secret，填入上方两个输入框", "同一机器人只能一个客户端长连接（本插件 / OpenClaw 二选一占用）"] }),
          CA("cb-wx-s2", "wecom", null, "保存 / 测试方案二", "2", true))),
      React.createElement(SettingsGroup, { title: "QQ", hint: "方案一为 OneBot HTTP 上报 + 出站；方案二为 QQ 开放平台 v2（官方机器人）。", group: "cb-qq" },
        React.createElement(Row, { label: "启用" },
        ...
        React.createElement("div", { "data-save-group": "cb-qq-s1" },
          React.createElement(SectionField, { label: "方案一 · OneBot HTTP 地址", hint: "入站上报地址（QQ 机器人在 OneBot 端填这个回调）。" },
            React.createElement(Field, { value: (value.qq && value.qq.onebotUrl) || "", onCommit: (v) => writeChannel(bound, value, "qq", { onebotUrl: v }), ariaLabel: "QQ OneBot 地址", field: "onebotUrl", fieldType: "text", placeholder: "https://your-domain.example.com/api/channel-bot/webhook/qq" })),
          React.createElement(SectionField, { label: "方案一 · Access Token", hint: "OneBot 上报鉴权 token，留空=不鉴权。" },
            React.createElement(Field, { value: (value.qq && value.qq.accessToken) || "", onCommit: (v) => writeChannel(bound, value, "qq", { accessToken: v }), ariaLabel: "QQ Access Token", field: "accessToken", fieldType: "text", placeholder: "OneBot 配置里的 access_token" })),
          React.createElement(SectionField, { label: "方案一 · 群号", hint: "通知/测试默认发到该群，留空=私聊。" },
            React.createElement(Field, { value: (value.qq && value.qq.groupId) || "", onCommit: (v) => writeChannel(bound, value, "qq", { groupId: v }), ariaLabel: "QQ 群号", field: "groupId", fieldType: "text", placeholder: "如 123456789" })),
          CA("cb-qq-s1", "qq", null, "保存 / 测试方案一", "1", true)),
        React.createElement("div", { "data-save-group": "cb-qq-s2" },
          React.createElement(SectionField, { label: "方案二 · QQ 开放平台 v2（填入 AppID+AppSecret 后本插件直接收发）", hint: "官方 QQ 开放平台机器人：AppID + AppSecret 换取 access_token（v2 OpenAPI），被动回复自动带收到的 msg_id。填了后优先于上方方案一 OneBot。管理端「开发基础设置」取 AppID/AppSecret；收消息用事件订阅 webhook（回调地址=上面 OneBot HTTP 地址）。群消息用机器人所在群的 group_openid，单聊需在管理端申请「主动消息/单聊发送」权限。" },
            React.createElement(Field, { value: (value.qq && value.qq.appId) || "", onCommit: (v) => writeChannel(bound, value, "qq", { appId: v }), ariaLabel: "QQ 开放平台 AppID", field: "appId", fieldType: "text", placeholder: "QQ 开放平台 AppID（数字）" }),
            React.createElement(Field, { value: (value.qq && value.qq.appSecret) || "", onCommit: (v) => writeChannel(bound, value, "qq", { appSecret: v }), ariaLabel: "QQ 开放平台 AppSecret", field: "appSecret", fieldType: "text", placeholder: "QQ 开放平台 AppSecret（管理端复制）" })),
          React.createElement(CredHelp, { label: "QQ 开放平台", href: "https://q.qq.com", hrefLabel: "打开 QQ 开放平台", steps: ["登录 QQ 开放平台（q.qq.com）→ 应用管理 → 你的机器人", "到「开发基础设置」复制 AppID 与 AppSecret，填入上方两个输入框", "到机器人「事件订阅」配置 webhook 回调 = 方案一 OneBot 地址（https://your-domain.example.com/api/channel-bot/webhook/qq）", "把服务器公网 IP 加进「IP 白名单」；单聊主动推送需申请「主动消息/单聊发送」权限"] }),
          CA("cb-qq-s2", "qq", null, "保存 / 测试方案二", "2", true))),
      ...
      CA("cb-wc", "wechat", null, "保存 / 测试微信（iLink 方案一）", "1", true),
      React.createElement("div", { "data-save-group": "cb-wc-s2" },
        React.createElement(SectionField, { label: "方案二 · 微信 OpenClaw / ClawBot 模式（仅记录）", hint: "OpenClaw 生态的微信机器人模式：在 OpenClaw 侧生成 Bot ID 与 Secret 填入此处。本插件仅做配置记录/展示（不直接收发），实际收发由 OpenClaw 侧承担；方案一（iLink Bot Token）不受影响且仍为默认通道。" },
          React.createElement(Field, { value: (value.wechat && value.wechat.botId) || "", onCommit: (v) => writeChannel(bound, value, "wechat", { botId: v }), ariaLabel: "微信 OpenClaw Bot ID", field: "botId", fieldType: "text", placeholder: "OpenClaw 微信机器人 Bot ID（仅记录）" }),
          React.createElement(Field, { value: (value.wechat && value.wechat.secret) || "", onCommit: (v) => writeChannel(bound, value, "wechat", { secret: v }), ariaLabel: "微信 OpenClaw Secret", field: "secret", fieldType: "text", placeholder: "OpenClaw 微信机器人 Secret（仅记录）" })),
        React.createElement(CredHelp, { label: "微信 OpenClaw 模式", href: "https://weixin.qq.com", hrefLabel: "打开微信官网", steps: ["方案二 = OpenClaw 生态的微信机器人（官方 iLink / ClawBot）", "在 OpenClaw 侧创建微信机器人，复制 Bot ID 与 Secret", "把两者填入上方输入框并点「保存」（仅作配置记录/展示）", "本插件不直接收发该机器人消息；微信收发仍走方案一 iLink Bot Token"] }),
        CA("cb-wc-s2", "wechat", null, "保存 / 校验方案二（记录型）", "2", false))),
      ...
      React.createElement(SettingsGroup, { title: "诊断" },
        React.createElement(Row, { label: "全渠道双向自检 + 分渠道修复", hint: "入站 = 收消息，出站 = 回复/推送；六个渠道各自的接收通道状态、最近入站与出站结果，可单独自检（发一条测试消息）。每条渠道还能「修复」：自动重启它的接收通道（轮询/长连接）、清除该渠道的访问令牌缓存与历史错误计数，并真打一次平台接口校验凭据；顶部「一键修复全部」按顺序修复所有已启用渠道。下表每 6 小时自动刷新（打开诊断页会立即刷新一次，也可点「刷新自检」手动刷新）。" }),
        React.createElement(ChannelHealthBlock, { bound: bound, value: value }),
        React.createElement(Row, { label: "服务端状态（原始 JSON）", hint: "查看面板侧实际启用的渠道与运行信息。" },
          React.createElement("button", {
  ...
  /* ---------- 全渠道双向自检（v2.2.0）+ 分渠道修复 / 一键修复全部（v2.3.0）----------
   * 渠道的轮询/长连接是「静默进程」：出问题时往往一条日志都没有。这里把
   * /api/channel-bot/status 的 health 账渲染成表，**六个渠道全覆盖**
   * （Telegram / 钉钉 / 飞书 / 企业微信 / QQ / 微信），并且：
   *   · 分渠道设定「是否参与自检」（关掉的渠道不出现在表里，也不参与「全部自检」）；
   *   · 每行一个「自检」按钮 → 单独给该渠道发一条测试消息并就地显示结果；
   *   · 每行一个「修复」按钮 → 自动重启该渠道的接收通道（轮询/长连接）、清访问令牌缓存、
   *     清历史错误计数，并真打一次平台接口校验凭据；结果就地显示，仍需人工的事项照实列出；
   *   · 顶部「一键修复全部」→ 对所有已启用渠道依次执行上面的修复；
   *   · 顶部「全部自检」→ 依次自检所有已启用且参与的渠道。 */
  var CH_LABELS = { telegram: "Telegram", dingtalk: "钉钉", feishu: "飞书", wecom: "企业微信", qq: "QQ", wechat: "微信(iLink)" };
  var CH_ALL = ["telegram", "dingtalk", "feishu", "wecom", "qq", "wechat"];
  /* [v2.3.0] 诊断表自动刷新间隔 = 6 小时（渠道状态变化很慢，20 秒一刷既无用又刷日志）。
   * 打开诊断页时仍会**立即刷新一次**，点「刷新自检」也能随时手动刷新；6 小时只是兜底。 */
  var DIAG_REFRESH_MS = 6 * 60 * 60 * 1000;
  function diagRefreshText(at) {
    return "每 6 小时自动刷新（打开诊断页会立即刷新一次）" + (at ? "（最近 " + at + "）" : "");
  }
  ...
  function receiverText(c) {
    if (!c.enabled) return "未启用";
    if (!c.receiver) return "❌ 未运行";
    if (c.receiverKind === "webhook") return "✅ 回调就绪";
    if (c.receiverKind === "socket") return "✅ 长连接";
    return "✅ 轮询中";
  }
  /* 把 /repair 的单渠道结果压成「一行摘要 + 完整明细（title 悬浮看）」。
   * 纯函数，便于单测：不依赖 React / DOM。 */
  function repairSummary(r) {
    if (!r) return { text: "❌ 修复接口无返回", ok: false, full: "服务端没有返回结果" };
    if (r.skipped === true) {
      var why0 = ((r.manual || [])[0]) || "渠道未启用";
      return { text: "⏭ 已跳过：" + why0, ok: false, full: "跳过：" + ((r.manual || []).join("；") || why0) };
    }
    var acts = r.actions || [];
    var manual = r.manual || [];
    var head = r.ok === true ? "✅ 修复完成（复核通过）" : "⚠ 已执行修复，但仍有问题";
    var text = head + (acts.length ? "：" + acts[0] : "") + (manual.length ? " ｜ 仍需人工：" + manual[0] : "");
    var full = [head].concat(acts).concat(manual.map(function (m) { return "❗ " + m; })).join("\n");
    return { text: text, ok: r.ok === true, full: full };
  }
  /* 一键修复全部的结果汇总（一行） */
  function repairAllSummary(b) {
    if (!b || b.ok === undefined) return "❌ 修复接口无返回";
    var s = b.summary || {};
    var parts = [];
    if (s.healthy) parts.push("✅ 修复并通过 " + s.healthy + " 个");
    if (s.failed) parts.push("⚠ 仍需人工 " + s.failed + " 个");
    if (s.skipped) parts.push("⏭ 跳过 " + s.skipped + " 个（未启用/凭据缺失）");
    parts.push("共执行 " + (s.actions || 0) + " 项动作");
    return parts.join("；") + (b.error ? " ｜ " + b.error : "");
  }
  function ChannelHealthBlock(props) {
    var bound = props && props.bound;
    var value = props && props.value;
    var st = React.useState(null); var h = st[0]; var setH = st[1];
    var se = React.useState(""); var err = se[0]; var setErr = se[1];
    var sa = React.useState(""); var at = sa[0]; var setAt = sa[1];
    var sr = React.useState({}); var results = sr[0]; var setResults = sr[1];
    var sb = React.useState(""); var busy = sb[0]; var setBusy = sb[1];
    /* [v2.3.0] 修复状态：repairing 为空 = 空闲；非空时按钮禁用（避免并发修复同一个渠道） */
    var srb = React.useState(""); var repairing = srb[0]; var setRepairing = srb[1];
    var srm = React.useState(""); var repairMsg = srm[0]; var setRepairMsg = srm[1];
    var srd = React.useState({}); var repairDetail = srd[0]; var setRepairDetail = srd[1];
    var load = function () {
      fetch("/api/channel-bot/status", { cache: "no-store" })
        .then(function (r) { return r.json(); })
        .then(function (b) { setH(b); setErr(""); setAt(new Date().toLocaleTimeString()); })
        .catch(function (e) { setErr(String(e)); });
    };
    React.useEffect(function () { load(); var iv = setInterval(load, DIAG_REFRESH_MS); return function () { clearInterval(iv); }; }, []);
    var runOne = function (ch) {
      setResults(function (r) { var n = Object.assign({}, r); n[ch] = "发送中…"; return n; });
      return fetch("/api/channel-bot/test", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ channel: ch }) })
        .then(function (r) { return r.json().catch(function () { return {}; }); })
        .then(function (b) {
          setResults(function (r) {
            var n = Object.assign({}, r);
            n[ch] = b && b.ok === true ? "✅ 已发送，请查收" : ("❌ " + ((b && b.error) || "失败"));
            return n;
          });
          return !!(b && b.ok === true);
        })
        .catch(function (e) {
          setResults(function (r) { var n = Object.assign({}, r); n[ch] = "❌ " + String(e); return n; });
          return false;
        });
    };
    var runAll = function (list) {
      ...
      var i = 0;
      var next = function () {
        if (i >= list.length) { setBusy(""); load(); return; }
        var ch = list[i++];
        runOne(ch).then(next);
      };
      next();
    };
    /* [v2.3.0] 单渠道修复：POST /api/channel-bot/repair {channel}，把结果就地显示在「操作」列 */
    var repairOne = function (ch) {
      setRepairMsg("");
      setRepairing(ch);
      return fetch("/api/channel-bot/repair", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ channel: ch }) })
        .then(function (r) { return r.json().catch(function () { return { ok: false, error: "HTTP " + r.status }; }); })
        .then(function (b) {
          var row = b && b.results && b.results.length ? b.results[0] : null;
          var s = row ? repairSummary(row) : { text: "❌ " + ((b && b.error) || "失败"), ok: false, full: String((b && b.error) || "") };
          setRepairDetail(function (d) { var n = Object.assign({}, d); n[ch] = s.full; return n; });
          setResults(function (r) { var n = Object.assign({}, r); n[ch] = s.text; return n; });
          return s.ok;
        })
        .catch(function (e) {
          setResults(function (r) { var n = Object.assign({}, r); n[ch] = "❌ 修复请求失败：" + String(e); return n; });
          return false;
        })
        .finally(function () { setRepairing(""); });
    };
    /* 一键修复全部已启用渠道：一次请求由服务端遍历（比前端循环更省往返，也更一致） */
    var repairAll = function () {
      setRepairMsg("一键修复中…");
      setRepairing("all");
      fetch("/api/channel-bot/repair", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ channel: "all" }) })
        .then(function (r) { return r.json().catch(function () { return { ok: false, error: "HTTP " + r.status }; }); })
        .then(function (b) {
          var list = (b && b.results) || [];
          if (list.length) {
            setResults(function (r) {
              var n = Object.assign({}, r);
              list.forEach(function (row) { if (row && row.channel) n[row.channel] = repairSummary(row).text; });
              return n;
            });
            setRepairDetail(function (d) {
              var n = Object.assign({}, d);
              list.forEach(function (row) { if (row && row.channel) n[row.channel] = repairSummary(row).full; });
              return n;
            });
          }
          setRepairMsg(repairAllSummary(b));
        })
        .catch(function (e) { setRepairMsg("❌ 一键修复请求失败：" + String(e)); })
        .finally(function () { setRepairing(""); load(); });
    };
    var cell = { padding: "5px 8px", borderBottom: "1px solid var(--dsw-alias-border-l2,#2a2a2a)", fontSize: 12, verticalAlign: "top" };
    var head = { padding: "5px 8px", borderBottom: "1px solid var(--dsw-alias-border-l2,#333)", fontSize: 12, fontWeight: 600, opacity: 0.85, textAlign: "left" };
    var btn = { padding: "4px 9px", borderRadius: 8, border: "1px solid var(--dsw-alias-border-l2, #333)", background: "transparent", color: "inherit", cursor: "pointer", fontSize: 11 };
    var chipOn = { padding: "3px 8px", borderRadius: 999, fontSize: 11, cursor: "pointer", border: "1px solid var(--dsw-alias-brand-primary,#315efb)", background: "transparent", color: "inherit" };
    var chipOff = { padding: "3px 8px", borderRadius: 999, fontSize: 11, cursor: "pointer", border: "1px solid var(--dsw-alias-border-l2,#333)", background: "transparent", color: "inherit", opacity: 0.6 };
    var meta = (h && h.channelsMeta) || CH_ALL.map(function (ch) { return { id: ch, label: CH_LABELS[ch], enabled: false, selfCheck: true }; });
    var health = (h && h.health) || {};
    var shown = meta.filter(function (m) { return m.selfCheck !== false; });
    var hiddenCount = meta.length - shown.length;
    var enabledShown = shown.filter(function (m) { return m.enabled; }).map(function (m) { return m.id; });
    /* [v2.3.0] 「有故障」= 健康账结论 ok=false（凭据缺失 / 接收通道未运行 / 连续失败 / 最近出站失败） */
    var problemShown = enabledShown.filter(function (ch) { var c = health[ch]; return !!c && c.ok === false; });
    var rows = shown.map(function (m) {
      var c = health[m.id] || { enabled: m.enabled, receiver: false, inbound: {}, outbound: {}, problems: [], notes: [] };
      var inbound = c.inbound && c.inbound.count > 0
        ? (agoText(c.inbound.lastAt) + " · " + String(c.inbound.lastFrom || "").slice(0, 16) + " · " + String(c.inbound.lastText || "").slice(0, 18))
        : (c.enabled ? "本次运行暂无入站" : "未启用");
      if (c.inbound && c.inbound.skipped > 0) inbound += "（跳过 " + c.inbound.skipped + "）";
      var outbound = c.outbound && c.outbound.count > 0
        ? ((c.outbound.lastOk === true ? "✅ " + c.outbound.ok + " 次" : "❌ " + String(c.outbound.lastError || "失败").slice(0, 34)) + " · " + agoText(c.outbound.lastAt))
        : "—";
      return React.createElement("tr", { key: m.id, "data-ch-health": m.id },
        React.createElement("td", { style: cell }, m.label + (m.scheme ? "（方案" + m.scheme + "）" : "")),
        React.createElement("td", { style: cell }, receiverText(c)),
        React.createElement("td", { style: cell }, inbound),
        React.createElement("td", { style: cell }, outbound),
        React.createElement("td", { style: { ...cell, color: (c.ok && !(c.notes || []).length) ? "inherit" : "#e0a33a" } },
          c.ok ? ((c.notes || []).length ? "✅ 正常（" + c.notes[0] + "）" : "✅ 正常") : "⚠ " + ((c.problems || []).join("；") || "见左列")),
        React.createElement("td", { style: cell },
          React.createElement("button", { type: "button", style: btn, title: "给该渠道单独发一条自检消息", "data-selfcheck": m.id, onClick: function () { runOne(m.id); } }, "自检"),
          React.createElement("button", { type: "button", style: { ...btn, marginLeft: 4 }, "data-repair": m.id, disabled: !!repairing,
            title: "自动修复该渠道：重启接收通道（轮询/长连接）+ 清访问令牌缓存 + 清历史错误计数 + 真打一次平台接口校验凭据",
            onClick: function () { repairOne(m.id); } }, repairing === m.id ? "修复中…" : "修复"),
          results[m.id] ? React.createElement("div", { style: { fontSize: 10, opacity: 0.85, marginTop: 3, maxWidth: 180, wordBreak: "break-all" }, title: repairDetail[m.id] || "" }, results[m.id]) : null));
    });
    return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
      React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" } },
        React.createElement("span", { style: { fontSize: 12, opacity: 0.8 } }, "参与自检的渠道："),
        meta.map(function (m) {
          var on = m.selfCheck !== false;
          return React.createElement("button", {
            key: m.id, type: "button", style: on ? chipOn : chipOff, "data-selfcheck-toggle": m.id,
            title: on ? "点击移出自检表" : "点击纳入自检表",
            onClick: function () { if (bound && value) writeChannel(bound, value, m.id, { selfCheck: !on }); }
          }, (on ? "✓ " : "○ ") + m.label);
        })),
      React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" } },
        React.createElement("button", { type: "button", style: btn, onClick: load }, "刷新自检"),
        React.createElement("button", { type: "button", style: btn, disabled: !!busy || enabledShown.length === 0, onClick: function () { runAll(enabledShown); }, "data-selfcheck-all": "1" },
          busy || ("全部自检（" + enabledShown.length + " 个已启用渠道）")),
        React.createElement("button", { type: "button", disabled: !!repairing || (!repairing && enabledShown.length === 0),
          style: { ...btn, borderColor: problemShown.length > 0 ? "#e0a33a" : undefined, color: problemShown.length > 0 ? "#e0a33a" : "inherit" },
          "data-repair-all": "1", onClick: repairAll,
          title: "对所有已启用渠道逐个执行修复：重启接收通道（轮询/长连接）+ 清访问令牌缓存 + 清历史错误计数 + 真打一次平台接口校验凭据" },
          repairing === "all" ? "一键修复中…"
            : (problemShown.length > 0
              ? ("一键修复全部（" + problemShown.length + " 个有故障 / 共 " + enabledShown.length + " 个已启用）")
              : ("一键修复全部（" + enabledShown.length + " 个已启用渠道）"))),
        React.createElement("span", { style: { fontSize: 11, opacity: 0.7 } },
          "每 20 秒自动刷新" + (at ? "（最近 " + at + "）" : "") + (err ? " ｜ 读取失败：" + err : ""))),
      repairMsg ? React.createElement("div", { "data-repair-msg": "1", style: { fontSize: 11, lineHeight: 1.7, padding: "6px 8px", borderRadius: 8, border: "1px dashed var(--dsw-alias-border-l2,#333)" } }, "修复结果：" + repairMsg) : null,
      React.createElement("table", { style: { width: "100%", borderCollapse: "collapse" } },
        React.createElement("thead", null, React.createElement("tr", null,
          React.createElement("th", { style: head }, "渠道"),
          React.createElement("th", { style: head }, "接收通道"),
          React.createElement("th", { style: head }, "最近入站"),
          React.createElement("th", { style: head }, "最近出站"),
          React.createElement("th", { style: head }, "结论"),
          React.createElement("th", { style: head }, "操作"))),
        React.createElement("tbody", null, rows.length ? rows : React.createElement("tr", null, React.createElement("td", { style: cell, colSpan: 6 }, "读取中…")))),
      hiddenCount > 0 ? React.createElement("div", { style: { fontSize: 11, opacity: 0.7 } }, "（" + hiddenCount + " 个渠道已移出自检表，点上方渠道名可重新纳入）") : null,
      h && h.state ? React.createElement("div", { style: { fontSize: 11, opacity: 0.75, lineHeight: 1.7 } },
        "通知目标：" + ((h.state.notifyTargets || []).map(function (k) { return CH_LABELS[k] || k; }).join("、") || "无"),
        (h.state.notifySkipped || []).length ? React.createElement("div", null, "未进通知列表的原因：", (h.state.notifySkipped || []).join("；")) : null,
        React.createElement("div", null, "微信 context_token 缓存：" + (h.state.wechatContexts || 0) + " 个会话"
          + (h.state.qqInbound ? "；QQ 最近入站：" + h.state.qqInbound.id : ""))) : null);
  }
  ...
  /* ---------- 注册到原生设置页 ---------- */
  function apply(ctx) {
    /* [2026-09-10] 采集宿主判定（只读，不改变行为），供设置不可用诊断面板显示原因 */
    try {
      var rd = (ctx !== null && ctx !== void 0 && ctx.remote !== null && ctx.remote !== void 0
        && ctx.remote.$host !== null && ctx.remote.$host !== void 0) ? ctx.remote.$host.isLoopback === true : null;
      hostDiag = { isLoopback: rd, origin: (typeof location !== "undefined" && location !== null) ? String(location.origin) : "" };
    } catch { /* keep defaults */ }
    ...
    if (ctx !== null && typeof ctx.settingsScope?.bind === "function") {
      ...
      store._set(scope);
      /* 供诊断面板的「重新绑定」按钮调用（作用域可能因热重载/晚到而需要重新获取） */
      rebindHook = () => {
        try { store._set(ctx.settingsScope.bind({ namespace: NS })); }
        catch { /* ignore */ }
      };
      ctx.effect(() => () => {
  ...
  /* [v2.3.0] 把修复结果的纯函数挂在 apply 上，便于 scripts/client.test.mjs 无浏览器单测 */
  apply.__repair = { repairSummary, repairAllSummary };
  /* [v2.3.0] 诊断表刷新间隔（6 小时）与文案也导出，锁住「不许退回秒级刷新」 */
  apply.__diag = { REFRESH_MS: DIAG_REFRESH_MS, refreshText: diagRefreshText };
