...
window.__ModuleLoader__.load({ id: "dsh-channel-bot", factory: (require) => {
  ...
  function ChannelBotSection() {
    ...
    return React.createElement(FoldContext.Provider, { value: foldValue },
      ...
      React.createElement(SettingsGroup, { title: "诊断" },
        React.createElement(Row, { label: "四渠道双向自检", hint: "入站 = 收消息，出站 = 回复/推送；每个渠道的接收通道是否在跑、最近一次入站与出站结果，下表每 20 秒自动刷新。" }),
        React.createElement(ChannelHealthBlock),
  ...
  /* ---------- 四渠道双向自检（v2.1.0）----------
   * 渠道的轮询/长连接是「静默进程」：出问题时往往一条日志都没有（历史上微信长轮询被打断
   * 后永久停摆就是这样）。这里把 /api/channel-bot/status 的 health 账渲染成一张表：
   * 接收通道在不在跑、最近一次入站、最近一次出站结果、以及结论/原因 —— 不用翻日志。 */
  var CH_LABELS = { telegram: "Telegram", wecom: "企业微信", qq: "QQ", wechat: "微信(iLink)" };
  ...
  function ChannelHealthBlock() {
    ...
    var btn = { padding: "5px 10px", borderRadius: 8, border: "1px solid var(--dsw-alias-border-l2, #333)", background: "transparent", color: "inherit", cursor: "pointer", fontSize: 12 };
    var rows = [];
    if (h && h.health) {
      ["telegram", "wecom", "qq", "wechat"].forEach(function (k) {
        var c = h.health[k] || {};
        var inbound = c.inbound && c.inbound.count > 0
          ? (agoText(c.inbound.lastAt) + " · " + String(c.inbound.lastFrom || "").slice(0, 18) + " · " + String(c.inbound.lastText || "").slice(0, 24))
          : "本次运行暂无入站";
        if (c.inbound && c.inbound.skipped > 0) inbound += "（跳过 " + c.inbound.skipped + "）";
        var outbound = c.outbound && c.outbound.count > 0
          ? ((c.outbound.lastOk === true ? "✅ 成功 " + c.outbound.ok + " 次" : "❌ " + String(c.outbound.lastError || "失败").slice(0, 40)) + " · " + agoText(c.outbound.lastAt))
          : "—";
        rows.push(React.createElement("tr", { key: k, "data-ch-health": k },
          React.createElement("td", { style: cell }, CH_LABELS[k]),
          React.createElement("td", { style: cell }, c.enabled ? (c.receiver ? "✅ 运行中" : "❌ 未运行") : "未启用"),
          ...
          React.createElement("td", { style: { ...cell, color: (c.ok && !(c.notes || []).length) ? "inherit" : "#e0a33a" } },
            c.ok
              ? ((c.notes || []).length ? "✅ 正常（" + c.notes[0] + "）" : "✅ 正常")
              : "⚠ " + ((c.problems || []).join("；") || "见左列"))));
      });
    }
    return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
      ...
      React.createElement("table", { style: { width: "100%", borderCollapse: "collapse" } },
        ...
        React.createElement("tbody", null, rows.length ? rows : React.createElement("tr", null, React.createElement("td", { style: cell, colSpan: 5 }, "读取中…")))),
      h && h.state ? React.createElement("div", { style: { fontSize: 11, opacity: 0.75, lineHeight: 1.7 } },
        ...
        (h.state.notifySkipped || []).length ? React.createElement("div", null, "跳过原因：", (h.state.notifySkipped || []).join("；")) : null,
